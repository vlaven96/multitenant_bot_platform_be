...
def get_user_statistics(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    """
    statistics_dto = SnapchatAccountStatisticsService.get_overall_statistics(db)
...
def get_statistics_grouped_by_model(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    """
    statistics_by_model = SnapchatAccountStatisticsService.get_overall_statistics_grouped_by_model(db)
...
def get_user_statistics(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    :return: SnapchatAccountStatsDTO containing the user's statistics.
    """
    statistics_dto = SnapchatAccountStatisticsService.get_average_time_for_all_statuses(db)

    return statistics_dto

@router.get("/average-times-by-source", response_model=Dict[str, str])
def average_times_by_source_exit(db: Session = Depends(get_db)):
    """
    ...
    """
    try:
        avg_times: Dict[str, Dict[str, timedelta]] = SnapchatAccountStatisticsService.get_average_time_by_source_for_status_exit(db)
...
def execution_counts_by_source(db: Session = Depends(get_db)):
    """
    Endpoint to retrieve the number of executions by account source
    until leaving RECENTLY_INGESTED or GOOD_STANDING.
    """
    try:
        counts = SnapchatAccountStatisticsService.get_execution_counts_by_source_until_status_change(db)
        return counts
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
